...
CREATE OR REPLACE VIEW user1_friends AS
SELECT u.id, u.firstname, u.lastname FROM (SELECT fr.initiator_user_id, fr.target_user_id FROM (SELECT * FROM users u WHERE u.id=1) u1
JOIN friend_requests fr ON (fr.initiator_user_id=1 OR fr.target_user_id=1) AND fr.status='approved') uf1
JOIN users u ON (u.id!=1 AND u.id=uf1.initiator_user_id) or (u.id!=1 AND u.id=uf1.target_user_id) ;
SELECT * FROM user1_friends;

-- 2. Создайте представление, в котором будут выводится все сообщения, в которых принимал
-- участие пользователь с id = 1.

CREATE OR REPLACE VIEW user1_massages AS
SELECT m.from_user_id, m.to_user_id, m.body FROM (SELECT * FROM users u WHERE u.id=1) u1
JOIN messages m ON from_user_id=1 OR to_user_id=1;
SELECT * FROM user1_massages;

-- 3.Получите список медиафайлов пользователя с количеством лайков(media m, likes l ,users u).
CREATE OR REPLACE VIEW users_media AS
...
GROUP BY m.id
ORDER BY u.id, total_likes DESC;
SELECT * FROM users_media;

-- 4. Получите количество групп у пользователей users_communities
SELECT u.id AS user_id,
...
WHERE date(p.birthday) BETWEEN  '2003-12-06' and now()
ORDER BY p.birthday;
SELECT * FROM users_under_20;

-- 6.Найдите кол-во, отправленных сообщений каждым пользователем и выведите ранжированный список пользователей, указав имя и фамилию 
-- пользователя, количество отправленных сообщений и место в рейтинге (первое место у пользователя с максимальным количеством сообщений) . (используйте DENSE_RANK)
SELECT u.id,
	u.firstname,
	u.lastname,
    COUNT(from_user_id) AS m_count,
    DENSE_RANK() OVER (ORDER BY COUNT(from_user_id) DESC) AS `dense_rank`
FROM users u JOIN messages m ON u.id=m.from_user_id
GROUP BY from_user_id;

-- 7.Выберите все сообщения, отсортируйте сообщения по возрастанию даты отправления (created_at) и найдите разницу дат отправления между 
-- соседними сообщениями, получившегося списка. (используйте LEAD или LAG)
SELECT * ,
TIMEDIFF(LEAD(created_at) OVER(ORDER BY created_at), created_at)
AS date_differance
FROM messages m;
