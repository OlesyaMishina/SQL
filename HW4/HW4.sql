...
SELECT cat_Murzik.*, s.*
FROM (SELECT `name`, id FROM cats WHERE name="Murzik") cat_Murzik
JOIN shops s ON s.id=cat_Murzik.shops_id;
...
SELECT new_orders.*, a.an_price
FROM (SELECT * FROM orders o where date(o.ord_datetime) 
BETWEEN  @date and adddate(@date , interval 7 day)) new_orders
JOIN analysis a ON a.an_id=new_orders.ord_an
ORDER BY new_orders.ord_datetime;
