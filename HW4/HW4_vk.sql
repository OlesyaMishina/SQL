...
set @date = date('2011-08-06');
   SELECT 
		CONCAT(firstname, " ", u.lastname) as full_name, # users
        p.birthday, #profiles
        COUNT(*) AS total_likes
	FROM `profiles` p
    JOIN likes l ON l.user_id=p.user_id
    JOIN users u ON u.id=l.user_id
    WHERE  date(p.birthday) BETWEEN  @date and now()
    GROUP BY l.user_id
    ORDER BY p.birthday DESC;
    
   # 2. Определить кто больше поставил лайков (всего): мужчины или женщины    
   SELECT gender_likes.gender,
   SUM(gender_likes.total_likes) AS Количество
   FROM
   (SELECT 
        p.gender, #profiles
        COUNT(*) AS total_likes
    ...
    JOIN likes l ON l.user_id=p.user_id
    JOIN users u ON u.id=l.user_id
    GROUP BY l.user_id) gender_likes
    GROUP BY gender_likes.gender
    ORDER BY Количество DESC
    LIMIT 1;
    
    #3.Вывести всех пользователей, которые не отправляли сообщения
    SELECT CONCAT(firstname, ' ', lastname) AS 'Не отправили сообщения',
    u.id
	FROM users u
	WHERE NOT EXISTS (
		SELECT from_user_id
		FROM messages
		WHERE u.id = messages.from_user_id
	);
